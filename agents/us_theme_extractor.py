...
{{
  ...
  "japan_plays": [
    {{
      "theme": "<テーマ名>",
      "stocks": ["<銘柄名(証券コード4桁)>", "<銘柄名(証券コード4桁)>", "<銘柄名(証券コード4桁)>"],
...
"""
    ...
    try:
        ...
        response = client.messages.create(
            model=CLAUDE_MODEL,
            max_tokens=4000,  # 激しい相場日のレスポンス切断を防ぐため増量
        ...
        logger.warning(f"JSONパースエラー（1回目）: {e} - リトライします")
        # ---- 1回リトライ ----
        try:
            ...
            logger.error(f"JSONパースエラー（リトライ後も失敗）: {e2}\n生テキスト: {raw_text[:400]}")
