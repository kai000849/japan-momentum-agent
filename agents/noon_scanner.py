...
# 前場維持判定の閾値（モード別・急騰終値からの許容下落率）
HOLD_THRESHOLD_BY_MODE = {
    "SHORT_TERM": 0.97,   # 早期エントリー型: -3%で失速とみなす
    "MOMENTUM":   0.95,   # 中長期トレンド型: 多少の押し目は許容
    "EARNINGS":   0.97,   # 業績起点型: SHORT_TERMと同等
}
HOLD_THRESHOLD_DEFAULT = 0.97  # モード不明時のフォールバック
...
def _judge_holding(intraday: dict, scan_close: float, mode: str = "SHORT_TERM") -> tuple[str, list]:
    """
    ...
    hold_threshold = HOLD_THRESHOLD_BY_MODE.get(mode, HOLD_THRESHOLD_DEFAULT)
    reasons = []
...
def run_noon_scan(scan_date: Optional[str] = None) -> list:
    ...
    for code in target_codes:
        ...
        judgment, reasons = _judge_holding(intraday, scan_close, mode)
