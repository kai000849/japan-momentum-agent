...
def _fetch_stock_news(company_name: str, stock_code: str, max_items: int = 4) -> list:
    ...
    ノイズ（Yahoo FinanceのAI解説・掲示板・フィードタイトル）を除外して返す。
    """
    # ノイズパターン: Claude に渡しても意味のない記事タイトル
    _NOISE_PATTERNS = [
        "Google ニュース",
        "AIが解説",
        "値動きの背景",
    ...
    try:
        ...
        news = []
        for t in titles[1:]:  # 先頭はフィードタイトルのためスキップ
...
{stocks_text}
【判断ルール】
1. whyCategory（急騰理由）: 上記の開示・ニュースに記載された事実のみを根拠にすること。
   「開示・ニュースなし」の銘柄は必ず「不明」を選択する（推測・創作禁止）。
   選択肢: 決算 / 業績修正 / 自社株買い・増配 / 資本提携・M&A / 受注・契約 / テーマ相乗り / ニュース / 需給 / 不明
2. entryTiming（翌日エントリー可否）:
   - day2_go  : 開示・ニュースに明確な材料あり＋継続性高い → 積極エントリー推奨
   - day2_watch: 材料はあるが継続性不明 → 寄り付き様子見
   - day2_skip : 「開示・ニュースなし」の銘柄、または材料不明・弱い・過熱感あり → 見送り
3. structuralChange: 中長期的な上昇トレンドの初動か（true/false）
   「開示・ニュースなし」の銘柄は必ずfalseにすること。
4. surgeReason: [タグ]で始まる50文字以内の要約（タグ: [決算][開示][ニュース][需給][不明]）
   「開示・ニュースなし」の場合は「[不明] 材料特定不可・要確認」とすること。
